...
const defaultForm = () => ({
  title: '',
  subTitle: '',
  teacherId: authStore.profile?.userId ?? null,
...
async function fetchCourses() {
  const { data } = await getTeacherCourseList(query)
...
async function submitForm() {
  ...
  try {
    const payload = {
      ...form,
      teacherId: authStore.profile?.userId ?? form.teacherId,
      price: Number(form.price || 0)
    }

    if (selectedCoverFile.value) {
      const { data } = await uploadTeacherCourseCoverFile(selectedCoverFile.value)
