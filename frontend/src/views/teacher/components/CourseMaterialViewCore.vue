...
} from '@/api/teacherMaterial'

const route = useRoute()
const router = useRouter()
const routeBasePath = '/teacher/course-management/materials'
...
async function fetchMaterials() {
  ...
  const { data } = await getTeacherMaterialList(query)
...
async function openEdit(id) {
  const { data } = await getTeacherMaterialDetail(id)
  editingId.value = id
  resetForm()
  Object.assign(form, {
...
async function submitForm() {
  ...
  try {
    ...
    if (selectedUploadFile.value) {
      const { data } = await uploadTeacherMaterialFile(selectedUploadFile.value)
    ...
    if (editingId.value) {
      await updateTeacherMaterial(editingId.value, payload)
      ElMessage.success('资料已更新')
    } else {
      await createTeacherMaterial(payload)
...
.material-toolbar { display: flex; align-items: flex-start; justify-content: space-between; gap: 18px; margin-bottom: 18px; }
.material-course-info { min-width: 0; }
.material-course-title { font-size: 18px; font-weight: 700; }
...
.upload-tip { margin-top: 8px; font-size: 12px; color: var(--text-secondary); }
...
@media (max-width: 900px) { .material-toolbar { flex-direction: column; align-items: stretch; } }
</style>
