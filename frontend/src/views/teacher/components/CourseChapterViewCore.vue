  ...
  <div class="page-card">
    ...
    <div v-if="courseId" class="table-card section-card">
      <div class="section-toolbar">
        ...
        <el-button type="primary" @click="openSectionCreate" :disabled="!currentChapter">
          新增小节
        </el-button>
      </div>
...
import {
  ...
  getTeacherChapterTree,
  updateTeacherChapter,
  updateTeacherSection,
  uploadTeacherSectionContentFile
} from '@/api/teacherChapter'
import {
  createTeacherSectionContent,
  deleteTeacherSectionContent,
  getTeacherSectionContentDetail,
  getTeacherSectionContentList,
  updateTeacherSectionContent
} from '@/api/teacherSectionContent'

const route = useRoute()
const router = useRouter()
const routeBasePath = '/teacher/course-management/chapters'
...
const sectionForm = reactive({ title: '', isFreeTrial: 0 })
...
const editor = useEditor({
  ...
  extensions: [
    ...
    Placeholder.configure({ placeholder: '输入正文内容，可插入图片、链接、标题、列表等' })
  ],
  editorProps: {
...
const contentFileAccept = computed(() => {
  if (contentForm.contentType === 'VIDEO') return 'video/*,.mp4,.mov,.m4v,.webm,.avi,.mkv'
  if (contentForm.contentType === 'IMAGE') return 'image/*,.jpg,.jpeg,.png,.gif,.webp,.bmp,.svg'
  if (contentForm.contentType === 'PDF') return '.pdf,application/pdf'
  if (contentForm.contentType === 'PPT') return '.ppt,.pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation'
  return '*'
})

const contentUploadTip = computed(() => {
  if (contentForm.contentType === 'VIDEO') return '支持 mp4/mov/webm/avi/mkv，大小不超过 500MB'
  if (contentForm.contentType === 'PDF') return '支持 PDF 文件'
  if (contentForm.contentType === 'IMAGE') return '支持 jpg/png/webp/gif 等图片文件'
  if (contentForm.contentType === 'PPT') return '支持 PPT/PPTX 文件'
  return '支持上传单个附件文件'
})

function resetChapterForm() {
  Object.assign(chapterForm, { title: '' })
  editingChapterId.value = null
  chapterFormRef.value?.clearValidate()
}

function resetSectionForm() {
  Object.assign(sectionForm, { title: '', isFreeTrial: 0 })
...
function sectionContentTypeText(value) {
  return ({ VIDEO: '视频', RICH_TEXT: '富文本', IMAGE: '图片', PDF: 'PDF', PPT: 'PPT', FILE: '附件' }[value] || '未知')
}
...
async function fetchChapterTree() {
  ...
  try {
    const { data } = await getTeacherChapterTree(courseId.value)
    chapters.value = data || []
...
async function handleCourseChange(value) {
  router.replace({ path: routeBasePath, query: { courseId: value } })
  activeChapterId.value = null
...
function openChapterEdit(chapter) {
  ...
  Object.assign(chapterForm, { title: chapter.title })
  chapterDialogVisible.value = true
}
...
function openSectionEdit(section) {
  ...
  Object.assign(sectionForm, { title: section.title, isFreeTrial: section.isFreeTrial })
  sectionDialogVisible.value = true
}
...
function openContentCreate() {
  if (!currentContentSection.value) return
  resetContentForm()
  contentDialogVisible.value = true
}

async function openContentEdit(id) {
  const { data } = await getTeacherSectionContentDetail(id)
  resetContentForm()
...
async function submitChapter() {
  ...
  try {
    if (editingChapterId.value) {
      await updateTeacherChapter(editingChapterId.value, chapterForm)
      ElMessage.success('章节已更新')
    } else {
      await createTeacherChapter(courseId.value, chapterForm)
...
async function submitSection() {
  ...
  try {
    const payload = { title: sectionForm.title, isFreeTrial: sectionForm.isFreeTrial }
    if (editingSectionId.value) {
      await updateTeacherSection(editingSectionId.value, payload)
      ElMessage.success('小节已更新')
    } else {
      await createTeacherSection(activeChapterId.value, payload)
...
async function fetchSectionContents(sectionId = currentContentSection.value?.id) {
  ...
  try {
    const { data } = await getTeacherSectionContentList(sectionId)
...
async function submitContentForm() {
  ...
  try {
    ...
    if (selectedContentUploadFile.value) {
      const { data } = await uploadTeacherSectionContentFile(selectedContentUploadFile.value, contentForm.contentType)
    ...
    if (editingContentId.value) {
      await updateTeacherSectionContent(editingContentId.value, payload)
      ElMessage.success('内容项已更新')
    } else {
      await createTeacherSectionContent(currentContentSection.value.id, payload)
...
async function handleDeleteSection(section) {
  ...
  await deleteTeacherSection(section.id)
...
async function handleDeleteContent(id) {
  ...
  await deleteTeacherSectionContent(id)
...
function syncEditorContent(currentEditor = editor.value) {
  if (!currentEditor || contentForm.contentType !== 'RICH_TEXT') return
...
function insertPendingEditorImage(file) {
  if (!editor.value) return
  const objectUrl = URL.createObjectURL(file)
  pendingEditorImages.value.push({ file, objectUrl })
  editor.value.chain().focus().setImage({ src: objectUrl, alt: file.name || '' }).run()
}

async function uploadPendingEditorImages() {
  if (!editor.value || !pendingEditorImages.value.length) return
  for (const item of pendingEditorImages.value) {
    const { data } = await uploadTeacherSectionContentFile(item.file, 'IMAGE')
...
function replaceEditorImageSrc(sourceUrl, targetUrl) {
  ...
  contentForm.contentJson = JSON.stringify(editor.value.getJSON()).replace(new RegExp(escapedSource, 'g'), () => targetUrl)
  if (contentForm.contentJson) {
    editor.value.commands.setContent(JSON.parse(contentForm.contentJson))
  } else {
    editor.value.commands.setContent(contentForm.contentHtml || '')
...
function validateContentFile(file) {
  if (contentForm.contentType === 'VIDEO') return validateSectionVideoFile(file)
...
onMounted(async () => {
  ...
  if (Number.isFinite(routeCourseId) && routeCourseId > 0) {
    ...
    if (courseId.value) {
      router.replace({ path: routeBasePath, query: { courseId: courseId.value } })
    }
  }
  await fetchChapterTree()
})
...
watch(
  ...
  async (value) => {
    ...
    if (!Number.isFinite(nextId) || nextId <= 0 || nextId === courseId.value) return
...
<style scoped>
.search-card { background: #fff; border-radius: 10px; padding: 16px 20px 4px; margin-bottom: 12px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.04); }
.search-card :deep(.el-form-item) { margin-bottom: 12px; }
.table-card { background: #fff; border-radius: 10px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.04); padding-bottom: 20px; }
.table-card + .table-card { margin-top: 14px; }
.table-card :deep(.table-row:hover) { background: #f6f9ff !important; }
.section-card { overflow: hidden; }
.section-toolbar { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px 10px; }
.section-title { font-size: 16px; font-weight: 700; }
.section-material-toolbar { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 16px; }
.section-material-context { min-width: 0; }
.section-material-title { font-size: 16px; font-weight: 700; }
.section-material-meta { margin-top: 6px; color: #909399; font-size: 13px; }
.upload-field { width: 100%; }
.upload-input { width: 100%; padding: 10px 12px; border: 1px solid var(--el-border-color); border-radius: 8px; background: #fff; box-sizing: border-box; }
.upload-input::file-selector-button { margin-right: 12px; border: 0; border-radius: 6px; background: rgba(64, 158, 255, 0.12); color: #409eff; padding: 8px 12px; cursor: pointer; }
.upload-tip { margin-top: 8px; font-size: 12px; color: #909399; }
.upload-error { margin-top: 4px; font-size: 12px; color: var(--el-color-danger); }
.readonly-field { width: 100%; min-height: 40px; padding: 10px 12px; border: 1px solid var(--el-border-color); border-radius: 8px; background: #f8fafc; box-sizing: border-box; color: #303133; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.tiptap-editor { width: 100%; border: 1px solid var(--el-border-color); border-radius: 8px; overflow: hidden; background: #fff; }
.tiptap-toolbar { display: flex; flex-wrap: wrap; gap: 8px; padding: 10px 12px; border-bottom: 1px solid #ebeef5; background: #f8fafc; }
.tiptap-surface { min-height: 220px; }
.tiptap-surface :deep(.ProseMirror) { min-height: 220px; padding: 14px 16px; outline: none; color: #303133; line-height: 1.8; }
.tiptap-surface :deep(.ProseMirror p.is-editor-empty:first-child::before) { color: #a8abb2; content: attr(data-placeholder); float: left; height: 0; pointer-events: none; }
.tiptap-surface :deep(.ProseMirror h1), .tiptap-surface :deep(.ProseMirror h2), .tiptap-surface :deep(.ProseMirror h3) { margin: 14px 0 8px; }
.tiptap-surface :deep(.ProseMirror p) { margin: 8px 0; }
.tiptap-surface :deep(.ProseMirror ul), .tiptap-surface :deep(.ProseMirror ol) { margin: 8px 0; padding-left: 22px; }
.tiptap-surface :deep(.ProseMirror blockquote) { margin: 12px 0; padding: 8px 12px; border-left: 4px solid #bfdbfe; background: #f8fbff; color: #475569; }
.tiptap-surface :deep(.ProseMirror img) { max-width: 100%; border-radius: 6px; }
.empty-state { text-align: center; padding: 60px 0; }
.empty-state.compact { padding: 48px 0 36px; }
.empty-icon-box { width: 72px; height: 72px; margin: 0 auto 16px; border-radius: 50%; background: #f5f7fa; display: flex; align-items: center; justify-content: center; font-size: 28px; color: #c0c4cc; }
.empty-state p { color: #909399; font-size: 14px; }
@media (max-width: 900px) { .section-material-toolbar { flex-direction: column; align-items: stretch; } }
</style>
